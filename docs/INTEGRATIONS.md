...
| **gbe_fork / SLS fork** | `steam_settings\load_dlls\SteamCloudSave.dll` (auto `LoadLibraryW`); DLL now auto-detects steam/appid/shadowRoot so no config needed | shipped (tools/steamcloudsave) |
...
| **universal mount** | `tools\steamcloudsave\install.ps1` auto-detects the loader (SHIM / `load_dlls` / OST `[inject]`) and mounts SteamCloudSave.dll for you; DLL autodetects steam path + appid + shadowRoot at runtime | shipped (tools/steamcloudsave/install.ps1) |
...
| **BetterSteamTools (BST)** | **Dota 2 JS plugin (CEF UI only)** — cannot mount native DLLs into game processes; SCT plugs in at the data layer (`registry.json` / `shadow-status.json`) | doc only |
